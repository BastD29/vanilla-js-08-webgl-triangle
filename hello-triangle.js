...
function helloTriangle() {
  ...
  //These coordinates are structured in a flat array format,
  // meaning that each pair of elements in the array represents the x and y coordinates of a vertex.
  // Here's a breakdown of the coordinates:
  const triangleVertices = [
    // Top middle vertex
    0.0, // The x-coordinate is 0.0, meaning it's located at the center along the horizontal axis.
    0.5, // The y-coordinate is 0.5, placing it above the origin (0,0) on the vertical axis.
    // Bottom left vertex
    -0.5, // The x-coordinate -0.5 places it to the left of the center.
    -0.5, // The y-coordinate -0.5 positions it below the origin.
    // Bottom right vertex
    0.5, // The x-coordinate 0.5 indicates a position to the right of the center.
    -0.5, // The y-coordinate -0.5 puts it below the origin.
  ];
  // This triangle would look like an isosceles triangle with its base parallel to the x-axis and the apex pointing upwards, centered along the y-axis.
  // This structure is commonly used in computer graphics and game development to define shapes in a 2D or 3D space.
  const triangleGeoCpuBuffer = new Float32Array(triangleVertices);
  const triangleGeoBuffer = gl.createBuffer(); // JavaScript does not interact directly with the GPU. So we create a buffer on the GPU
  ...
  gl.clearColor(0.08, 0.08, 0.08, 1.0); //   clearColor(red, green, blue, alpha)
  // webgl does not just have one image that it uses that you are drawing to it
  // it actually has 3 buffers :
  // 1. the image, the actual picture that has all the colors that you are drawing things onto
  // 2. the depth buffer that stores depth information for each pixel on the output
  // 3. the stencil buffer that is used for certain graphic effects
  // gl.clear() clears the color buffer and the depth buffer
